...
class Issue < ActiveRecord::Base
  ...
  def self.count_and_group_by(options)
    ...
    ati = "issues.assigned_to_id"

    Issue.
      visible(User.current, :project => options[:project], :with_subprojects => options[:with_subprojects], :abc => options[:association]).
      joins(:status, assoc.name).
      where('issues.tracker_id <> 5 and issues.tracker_id <> 13').
      group(:status_id, :is_closed, select_field, ati).
      count.
      map do |columns, total|
      status_id, is_closed, field_value, issues_assigned_to_id, count_all = columns
      is_closed = ['t', 'true', '1'].include?(is_closed.to_s)
      {
          ...
          "total" => total.to_s,
          "row_assigned_to_id" => issues_assigned_to_id
      }
    end
  end
